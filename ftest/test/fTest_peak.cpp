...
RooAbsPdf* getPdf(string analysisBranch, RooRealVar * obsVar, PdfModelBuilder &pdfsModel, string type, int order, const char* ext=""){

  // COMBINATORIAL BACKGROUND
  RooAbsPdf * combBckg;
  if (type=="Bernstein") combBckg = pdfsModel.getBernstein(Form("%s_bern%d",ext,order),order);
  else if (type=="Chebychev") combBckg = pdfsModel.getChebychev(Form("%s_cheb%d",ext,order),order);
  else if (type=="Exponential") combBckg = pdfsModel.getExponentialSingle(Form("%s_exp%d",ext,order),order);
  else if (type=="PowerLaw") combBckg = pdfsModel.getPowerLawSingle(Form("%s_pow%d",ext,order),order);
  else if (type=="Laurent") combBckg = pdfsModel.getLaurentSeries(Form("%s_lau%d",ext,order),order);
  else {
    cerr << "[ERROR] -- getPdf() -- type " << type << " not recognised." << endl;
    return NULL;
  }

  if (!combBckg) return NULL;

  // PEAKING BACKGROUND
  RooRealVar * mean_dcbPeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_Mean").c_str(), "Mean" ,91.1876,70,120) ;
  RooRealVar * sigma_dcbPeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_Sigma").c_str(), "Width" ,  2., 0.5, 4.) ;
  RooRealVar * n1PeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_n1").c_str(),"", 0.5, 0.1, 50.);//dCBPowerL
  RooRealVar * n2PeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_n2").c_str(),"", 0.5, 0.1, 50.);//dCBPowerR
  RooRealVar * alpha1PeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_alpha1").c_str(),"", 3., 0.0, 10.);//dCBCutL
  RooRealVar * alpha2PeakingBackground = new RooRealVar(((string)(combBckg->GetName())+"_dcball_alpha2").c_str(),"", 3., 0.0, 10.);//dCBCutR

  RooDoubleCB * dcballPeakingBackground = new RooDoubleCB(
    // "dcballPeakingBackground",  
    ((string)(combBckg->GetName())+"_dcball").c_str(),  
    ...
    *n2PeakingBackground
    ...
    RooRealVar * dcb_fraction = new RooRealVar(((string)(combBckg->GetName())+"_dcball_fraction").c_str(), "dcb_fraction",  0.1, 0.0, 1.0);
    RooAbsPdf * totalBckg = new RooAddPdf(((string)(combBckg->GetName())+"_total").c_str(),"totalBckg",RooArgList(*combBckg, *dcballPeakingBackground), *dcb_fraction,true); 
...
// the one for multipdf plot
void plot(string analysisBranch, RooRealVar *mass, RooMultiPdf *pdfs, RooCategory *catIndex, RooDataSet *data, string name, vector<string> flashggCats_, int cat, int bestFitPdf=-1){
...
int main(int argc, char* argv[]){
  ...
  // is Z boson?
  if (isZ_) {
    cout << "-------> It a Z boson analysi!" << endl;
    mgg_low = 70;
    mgg_mid_low = 80;
    mgg_mid_high = 100;
    mgg_high = 120;
...
for (vector<string>::iterator funcType=functionClasses.begin(); funcType!=functionClasses.end(); funcType++) {
  ...
  while (prob<0.05 && order < 7) { //FIXME
    ...
    RooAbsPdf *bkgPdf = getPdf(analysisBranch, mass, pdfsModel,*funcType,order,Form("ftest_pdf_%d_%s",cat,ext.c_str()));
    if (!bkgPdf){
      ...
      if (prev_pdf!=NULL){
        prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,data ,Form("%s/Ftest_from_%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat));
  ...
  if (saveMultiPdf){
    ...
    while (prob<upperEnvThreshold){
      ...
      RooAbsPdf *bkgPdf = getPdf(analysisBranch, mass, pdfsModel,*funcType,order,Form("env_pdf_%d_%s",cat,ext.c_str()));
...
if (saveMultiPdf) {
  ...
  plot(analysisBranch, mass,pdf,&catIndex,data,Form("%s/multipdf_%s",outDir.c_str(),catname.c_str()),flashggCats_,cat,bestFitPdfIndex);
